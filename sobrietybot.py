...
token = os.environ.get('SOBRIETYBOT_TOKEN')
channel_id = os.environ.get('SOBRIETYBOT_CHANNEL_ID')
...
class MyClient(discord.Client):
    ...
    def days(self, username):
        cursor = db.cursor()
        cursor.execute(
            '''SELECT date from users where username = '{}';'''.format(username))
    ...
    def leaderboard(self):
        ...
        for i in records:
            count += 1
            try:
                current_days = (datetime.datetime.now()
                                - datetime.datetime.strptime(i[1], '%Y-%m-%d %H:%M:%S')).days
            except ValueError:
                print(ValueError)
                current_days = (datetime.datetime.now()
                                - datetime.datetime.strptime(i[1], '%Y-%m-%d')).days
            response += '''{}. {} - {} days\n'''.format(
                str(count), i[0], current_days)
        return response
    ...
    def list(self):
        ...
        for i in users:
            ...
            sobrietylist += str(count) + '. ' + username + \
                ' - ' + str(current_days) + ' days sober\n'
        cursor.close()
        db.commit()
        return sobrietylist
            ...
            for i in milestones:
                if days_sober <= i:
                    if days_sober == i and days_sober != last_milestone:
                        ...
                        query = """UPDATE users SET milestone = '{}' WHERE username = '{}';""".format(
                            days_sober, username)
                        cursor.execute(query)
                        db.commit()
        ...
        if client.user.mentioned_in(message):
            ...
            if command == 'set':
                if len(message.content.split()) == 3:  # e.g. @sobrietybot set 12/25/2020
                    setdate = message.content.split()[2].replace('/', '-')
            ...
            # DAYS
            if command == 'days' or command == 'info':
                sober_date, days_sober = self.days(user)
                plural = '' if days_sober == 1 else 's'
                await message.channel.send(f'You are {days_sober} day{plural} sober! You have been sober since {sober_date.strftime("%m-%d-%Y")}!')
